...
@Request.application
def trivial_app(request):
    """A trivial app that can be passed to RemoteDebugger's parent class"""
    return Response('You should never see this.')
...
class DebuggedApplication(object):
    ...
    def __init__(self, evalex=True, request_key='werkzeug.request',
                 console_path='/console', console_init_func=None,
                 show_hidden_frames=False, lodgeit_url=None,
                 pin_security=False, pin_logging=True):
    ...
    def get_resource(self, request, filename):
        ...
        filename = join(dirname(os.path.abspath(__file__)), 'resources', basename(filename))
        print(filename)
        if isfile(filename):
            print('yup')
